	   ...
	   present_handle : function(stim) {
	    	...
	    	var sentences = {
		    	must: "\"The bird behind the tree must be a "+ exp.bird_a+".\"",
		      	...
		      	probably: "\"The bird behind the tree is probably a "+ exp.bird_a+".\"",
  ...
  slides.bug_slider = slide({
	    ...
	    /* trial information for this block
	     (the variable 'stim' will change between each of these values,
	      and for each of these, present_handle will be run.) */
	    present : ([_.shuffle([
	    	{type: "indirect", evidence: "All he sees of the bug behind the leaf are its antennae:",color:"#FF0000",path: "m 5.419715,163.01459 c 11.961335,-3.66524 10.998423,-18.96404 15.179901,-30.92883 8.983951,-25.7065 34.010716,-45.08578 42.143043,-21.82588 L 51.922914,141.49734 C 64.326665,83.967056 31.442532,110.01215 25.42753,145.60325 21.993687,165.92146 11.888937,178.64831 5.087253,179.57328 z M 71.160253,18.211588 C 76.44653,29.55015 64.43836,39.078218 58.350997,50.195106 45.272234,74.079852 49.632881,106.09369 70.192991,96.47811 L 86.02947,64.834665 C 50.19598,120.57344 52.27524,65.855154 71.62388,44.730501 85.54198,29.534854 87.10394,12.549182 83.23691,6.877797 z"},
	    	{type: "direct", evidence: "All he sees of the bug behind the leaf is its stinger:",color:"#bbb",path: "M 216.83579,169.37132 248.5,171.11827 222.20711,157.27512 z"},
	    	{type: "report", evidence: "Somebody else looked behind the leaf and told Tom the bug was a "+exp.bug_a+".",path: "" }
	    	//{evidence: "report"}
	    	])]),
	    ...
	    present_handle : function(stim) {
	    	...
	    	var sentences = {
		    	must: "\"The bug behind the leaf must be a "+ exp.bug_a+".\"",
		      	...
		      	probably: "\"The bug behind the leaf is probably a "+ exp.bug_a+".\"",
		      	might: "\"The bug behind the leaf might be a "+ exp.bug_a+".\""
		      	},
		    sentence = sentences[exp.sentenceorder[1]]
	    	...
	    	$("#b_evidence1").html(stim[0].evidence);
		   	var paper = new Raphael(document.getElementById("bug_evidence1"),250,250);
			var p = stim[0].path;
			var mark = paper.path(p);
			mark.attr("fill",stim[0].color);

			$("#b_evidence2").html(stim[1].evidence);
		   	var paper = new Raphael(document.getElementById("bug_evidence2"),250,250);
			var p = stim[1].path;
			var mark = paper.path(p);
			mark.attr("fill",stim[1].color);

			$("#b_evidence3").html(stim[2].evidence);
		   	var paper = new Raphael(document.getElementById("bug_evidence3"),250,250);
			var p = stim[2].path;
			var mark = paper.path(p);
			mark.attr("fill",stim[2].color);	
	    ...
	    present_handle : function(stim) {
	    	...
	    	var sentences = {
		    	must: "\"The flower behind the rock must be a "+ exp.flower_a+".\"",
		      	...
		      	probably: "\"The flower behind the rock is probably a "+ exp.flower_a+".\"",
	...
	slides.fish_slider = slide({
	    ...
	    /* trial information for this block
	     (the variable 'stim' will change between each of these values,
	      and for each of these, present_handle will be run.) */
	    present : ([_.shuffle([
	    	{type: "indirect", evidence: "All he sees of the fish behind the clam are its fangs:",color:"",path: "m 224.72087,128.95491 6.81853,-2.25254 c 0.57697,22.03278 -2.82062,45.16585 -7.8363,63.4702 3.19194,-21.68019 6.0638,-45.94478 1.01777,-61.21766 z"},
	    	{type: "direct", evidence: "All he sees of the fish behind the clam are its whiskers:",color:"",path: "m 216.12695,132.08879 c -14.29361,23.85879 -34.75106,44.10429 -11.61675,89.90358 m 8.08122,-90.91373 c -30.52433,36.14586 -40.99373,68.74778 -24.74873,111.11677 m 22.72842,-114.14723 c -32.28674,23.34556 -51.60559,60.7522 -38.89087,107.07617"},
	    	{type: "report", evidence: "Somebody else looked behind the clam and told Bob the fish was a "+exp.fish_a+".",path: "" }
	    	//{evidence: "report"}
	    	])]),
	    //this gets run only at the beginning of the block
	    present_handle : function(stim) {
	    	...
	    	var sentences = {
		    	must: "\"The fish behind the clam must be a "+ exp.fish_a+".\"",
		      	...
		      	probably: "\"The fish behind the clam is probably a "+ exp.fish_a+".\"",
		      	might: "\"The fish behind the clam might be a "+ exp.fish_a+".\""
		      	},
		    sentence = sentences[exp.sentenceorder[3]]
	    	...
	    	$("#fi_evidence1").html(stim[0].evidence);
		   	var paper = new Raphael(document.getElementById("fish_evidence1"),250,250);
			var p = stim[0].path;
			var mark = paper.path(p);
			mark.attr("fill",stim[0].color);

			$("#fi_evidence2").html(stim[1].evidence);
		   	var paper = new Raphael(document.getElementById("fish_evidence2"),250,250);
			var p = stim[1].path;
			var mark = paper.path(p);
			mark.attr("fill",stim[1].color);

			$("#fi_evidence3").html(stim[2].evidence);
...
function init() {
  ...
  exp.sentenceorder = _.shuffle(["must","bare","probably","might"]); 
  exp.system = {
