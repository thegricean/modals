...
We capture these results in a lexical uncertainty model in which the semantics of the bare utterance and \textit{must} $q$ are relatively unconstrained, following \cite{bergen, lassiter}. We define the semantics of the utterances such that $p(q | \textit{bare} ) > \theta_b$ and $p(q | \textit{must} ) > \theta_m$, where the pragmatic listener is uncertain about $\theta_b$ and $\theta_m$ and infers the values through pragmatic reasoning. When the cost of uttering \textit{must} $q$ is greater than the bare form, the pragmatic listener infers that $p(q)$ is smaller than when the utterance is the less costly \textit{bare} $q$. % under certain prior distributions of $p(q)$. This provides evidence that the weakness of \textit{must} can arise from an M-implicature, where\textit{bare} is a salient and less costly alternative utterance. 
